...
from multaichat.chat import ChatPersona, ChatConversation, ChatGenerator, load_config
import multaichat.personas
...
def main(
    ...
    max_reply_len: int = 1,
    active_persona='LLaMaChat3',
):

    print('huh???', model_name, temperature)
    multaichat.personas.default_model_name = model_name
                ...
                if cargs[0] == 's':
                    ...
                    if len(cargs) > 1:
                        ...
                        for pname in personas:
                            p = personas[pname]
                            nextresponse = f'**{p.icon} {p.name}**\n{p.personality_prompt}\n\n'
                            if len(response) + len(nextresponse) > 2000:
                                await channel.send(response)
                                response = ''
